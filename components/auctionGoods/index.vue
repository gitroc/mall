		...
		<view class='goodWrapper pad30'>
			<view class='item acea-row row-between-wrapper' v-for="(item,index) in cartInfo" :key="index"
				...
				<view class='text'>
					...
					<view class='attr line1'>{{item.payNum ? item.payNum : item.cartNum}} 张{{item.ownerId ? ', 拥有者:' + item.ownerName + item.ownerMobile : ""}}</view>
					<view class='money-y'>￥{{item.price}}</view>
