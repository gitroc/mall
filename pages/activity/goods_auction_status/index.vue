<template>
	<view>
		<view class='payment-status'>
			...
			<view class='wrapper'>
				...
				<view class='item acea-row row-between-wrapper'>
                    ...
                    <view class='itemCom' v-else-if="order_pay_info.payType=='integral'">积分支付</view>
				</view>
				<view class='item acea-row row-between-wrapper'>
					<view>支付金额</view>
					<view class='itemCom'>{{order_pay_info.deductionPrice}}</view>
				</view>
				<!--失败时加上这个  -->
		...
		<!-- 	<view @tap="goOrderDetails" v-if="order_pay_info.paid==0 && status==1">
			...
			<button @click="goAuctionIndex" class='returnBnt cart-color' formType="submit" hover-class='none' v-else>返回竞拍</button>
			</view>
		<!-- #ifdef MP -->
...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 * 去首页关闭当前所有页面
			 */
			goAuctionIndex: function(e) {
				uni.switchTab({
					url: '/pages/activity/goodes_auction/index'
				});
			},
			// 去参团页面；
            goPink:function(id){
			...
			goOrderDetails: function(e) {
				...
				openOrderSubscribe().then(res => {
					uni.hideLoading();
					uni.navigateTo({
						url: '/pages/activity/goods_auction_detail/index?order_id=' + that.orderId
					});
				}).catch(() => {
					nui.hideLoading();
				});
